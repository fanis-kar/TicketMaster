...
namespace TicketMaster.IntegrationTests
{
    ...
    class TicketsControllerTests
    {
        ...
        [Test]
        public async Task RetrieveAllTicketsTest()
        {
            using var client = new HttpClient();
            var result = await client.GetAsync(new Uri(_ticketsRoute)).ConfigureAwait(false);
            Assert.That(result.StatusCode, Is.EqualTo(HttpStatusCode.OK));
        ...
        [Test]
        public async Task RetrieveTicketsShowExistsTest()
        {
            using var client = new HttpClient();
            ...
            Assert.IsTrue(savedTickets.All(t=>t.ShowId == _newShow1Id));           
        }

        [Test]
        public async Task RetrieveTicketsShowDoesNotExistTest()
        {
            using var client = new HttpClient();
            var result = await client.GetAsync(new Uri(_ticketsRoute + "show/"+long.MaxValue)).ConfigureAwait(false);
            await TestUtils.CheckForNotFound(result).ConfigureAwait(false);
        }

        [Test]
        public async Task ReserveTicketsCapacityExceededTest()
        {
            ...
            using var client = new HttpClient();
            var result = await TestUtils.Post(client, _ticketsRoute, ticketsRsv).ConfigureAwait(false);
            Assert.That(result.StatusCode, Is.EqualTo(HttpStatusCode.RequestedRangeNotSatisfiable));
            var error = JsonConvert.DeserializeObject<ErrorDetails>(await result.Content.ReadAsStringAsync().ConfigureAwait(false));
            Assert.True(error.Message.Contains("limit", StringComparison.InvariantCulture));
            Assert.True(error.StatusCode == (int)HttpStatusCode.RequestedRangeNotSatisfiable);
        }
        ...
        */
        [Test]
        public async Task ReserveTicketsWhenShowDoesNotExistTest()
        {
            var ticketsRsv = new TicketJSONRequest()
            {
                ShowId = long.MaxValue,
                Quantity = 3,
                Price = 30
            };
        ...
        [Test]
        public async Task DeleteTicketExistsTest()
        {
            using var client = new HttpClient();
            var result = await client.DeleteAsync(new Uri(_ticketsRoute+_ticketsLot1.First().Id)).ConfigureAwait(false);
            Assert.IsTrue(result.IsSuccessStatusCode);            
        }

        [Test]
        public async Task DeleteTicketDoesNotExistTest()
        {
            using var client = new HttpClient();
            var result = await client.DeleteAsync(new Uri(_ticketsRoute+long.MaxValue)).ConfigureAwait(false);
            await TestUtils.CheckForNotFound(result).ConfigureAwait(false);
        }
    }
}
